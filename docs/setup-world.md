...
For the **full** builder flow (environment → world → contract → scripts), follow one of the following guides: [Docker](./builder-flow-docker.md) or [Host](./builder-flow-host.md). The steps below cover only the world-contracts part; assume `world-contracts` and `builder-scaffold` are sibling directories.

### setup-world (one command)
...
This script clones world-contracts to `WORLD_CONTRACTS_DIR` if it doesn't exist (default `../world-contracts`), checks out `WORLD_CONTRACTS_BRANCH` (and optionally `WORLD_CONTRACTS_COMMIT`) in `.env`, deploys, configures, seeds, and copies artifacts. It ensures the deployed world matches the version your extensions build against.
...
**Docker bind mount:** When using the [Docker flow](./builder-flow-docker.md), world-contracts is bind-mounted. The script operates on `WORLD_CONTRACTS_DIR` (default `../world-contracts`). If the mount is empty, the script clones into it. Run from `/workspace/builder-scaffold`. See [builder-flow-docker.md](./builder-flow-docker.md#setup-world-script).
...
```bash
pnpm rebuild-world
# or: pnpm clean-world && pnpm setup-world
# or: pnpm setup-world --clean
```

For a fully fresh localnet, restart the node first (new chain, new chain ID), then rebuild:

**Docker:**
```bash
cd docker
docker compose down -v          # removes the sui-config volume — wipes keys and node state
docker compose run --rm --service-ports sui-dev   # new keys generated, new docker/.env.sui written
# update .env with new keys/addresses from docker/.env.sui, then:
cd ..
pnpm rebuild-world
```
...
**Host (no Docker):**
```bash
# Stop the running node first (Ctrl+C in its terminal, or kill the process)
# Then start a fresh chain:
sui start --with-faucet --force-regenesis
# (in a separate terminal) then:
pnpm rebuild-world
```

> After `rebuild-world`, re-publish all extensions and re-create any runtime objects (`pnpm create-marketplace`, `pnpm create-supply-unit`).
...
Use a **stable tag** so builder-scaffold stays compatible if world-contracts has breaking changes on the most recent version of `main`.
...
```bash
# Clone then checkout a specific ref — equivalent to the automated flow:
#   WORLD_CONTRACTS_BRANCH=main          → git checkout main   (tracks branch tip)
#   WORLD_CONTRACTS_COMMIT=v0.0.15       → git checkout v0.0.15 (pins to tag, detached HEAD)
#   WORLD_CONTRACTS_COMMIT=abc1234       → git checkout abc1234 (pins to SHA, detached HEAD)

git clone https://github.com/evefrontier/world-contracts.git
cd world-contracts
git checkout <latest-tag>   # e.g. v0.0.15
...
- Full flow in one place: [Docker](./builder-flow-docker.md) or [Host](./builder-flow-host.md)
