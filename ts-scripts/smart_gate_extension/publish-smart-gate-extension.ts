...
async function main() {
    ...
    try {
        ...
        if (isLocal && hasWorldPubFile) {
            ...
            const extPubFile = path.resolve(process.cwd(), "deployments", network, "Pub.smart-gate.localnet.toml");
        ...
        type ObjectChange = { type: string; objectType?: string; objectId?: string; packageId?: string };
        ...
        const extractedPath = upsertExtractedObjectIds(network, (d) => { d["smart_gate_extension"] = builderIds; });
