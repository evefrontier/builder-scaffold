#[test_only]
module tokens::tokens_tests;

use std::string::utf8;
use sui::{coin, test_scenario as ts, token};
use tokens::{currency_token, tribe_rule, tribe_token};
...
fun test_currency_token_open_transfer() {
    let mut ts = ts::begin(test_helpers::admin());
    {
        let mut treasury_cap = coin::create_treasury_cap_for_testing<
            currency_token::CURRENCY_TOKEN,
        >(ts::ctx(&mut ts));
        let c = coin::mint(&mut treasury_cap, MINT_AMOUNT, ts::ctx(&mut ts));
        transfer::public_transfer(c, test_helpers::user_a());
        transfer::public_transfer(treasury_cap, test_helpers::admin());
    };
    ts::next_tx(&mut ts, test_helpers::user_a());
    {
        let coin = ts::take_from_sender<coin::Coin<currency_token::CURRENCY_TOKEN>>(&ts);
        transfer::public_transfer(coin, test_helpers::user_b());
    };
    ts::next_tx(&mut ts, test_helpers::user_b());
    {
        let coin = ts::take_from_sender<coin::Coin<currency_token::CURRENCY_TOKEN>>(&ts);
        transfer::public_transfer(coin, test_helpers::user_b()); // keep with user_b
    };
    ts::end(ts);
}
...
#[test, expected_failure(abort_code = ::tokens::tribe_rule::ENotAllowedTribe)]
