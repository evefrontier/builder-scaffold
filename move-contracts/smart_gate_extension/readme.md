...
**World dependency:** Local path `../../../world-contracts/contracts/world` (see [setup-world](../../scripts/setup-world.sh) to pin branch/commit)
...
- `pnpm authorise-gate` – Authorizes `XAuth` on both gates and the storage unit (required for bounty path which deposits into the storage unit).
...
- World deployed and configured (see [setup-world](../docs/setup-world.md))
...
2. Run `pnpm authorise-gate` to authorize `XAuth` on both gates and the storage unit.
...
| `pnpm authorise-gate`          | Authorize XAuth on both gates + storage unit (one command covers all)            |
